...
import { FaUser, FaLock, FaEnvelope, FaPhone, FaSave, FaArrowLeft,FaEye, FaEyeSlash } from 'react-icons/fa';
import Footer from '../../components/common/Footer';
import { editPassword, editProfile, getProfileDetails } from '../../api/user_api';
                        ...
                        <form onSubmit={handleSubmitProfile(saveProfileChanges)} className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    {profileError.name && <p className='text-sm text-red-500'>{profileError.name.message}</p>}
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="relative rounded-md shadow-sm">
                                        ...
                                        <input
                                            ...
                                            id="currentPassword"
                                            {...registerPassword('currentPassword', { required: "Current password cannot be empty" })}
                                            className="input-field ms-4 border-light w-75"
                                        />
                                        <div className="absolute inset-y-0 right-0 pr-2 flex items-center cursor-pointer" onClick={() => setShowCurrentPassword(!showCurrentPassword)}>
                                            {showCurrentPassword ? <FaEyeSlash className="text-gray-400" /> : <FaEye className="text-gray-400" />}
                                        </div>
                                    </div>
                                    {passwordError.currentPassword && (
                                        <p className='text-sm text-red-500'>{passwordError.currentPassword.message}</p>
                                    )}
                                </div>
                                        ...
                                        <input
                                            ...
                                            {...registerPassword('newPassword', {
                                                ...
                                                pattern: {
                                                    ...
                                                    message: "Password must be 6-16 characters long and contain at least one number and one special character."
                                                }
                                            })}
                                            className="input-field ms-4 border-light w-75"
                                        />
                                        <div className="absolute inset-y-0 right-0 pr-2 flex items-center cursor-pointer" onClick={() => setShowNewPassword(!showNewPassword)}>
                                            {showNewPassword ? <FaEyeSlash className="text-gray-400" /> : <FaEye className="text-gray-400" />}
                                        </div>
                                    </div>
                                    {passwordError.newPassword && (
                                        <p className='text-sm text-red-500'>{passwordError.newPassword.message}</p>
                                    )}
                                </div>
                            </div>
