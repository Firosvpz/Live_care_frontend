...
import { FaUser, FaLock, FaEnvelope, FaPhone, FaSave, FaArrowLeft, FaEye, FaEyeSlash,FaBook } from 'react-icons/fa';
import Footer from '../../components/common/Footer';
import { editPassword, editProfile, fetchCategories, getSpProfileDetails } from '../../api/sp_api';
...
const ServiceProviderProfile: React.FC<EditSpProps> = ({ serviceProviderDetails, onProfileEdit }) => {
  const [categories, setCategories] = useState<string[]>([]);
  const [showPassword, setShowPassword] = useState(false);

  const { register, handleSubmit, setValue, formState: { errors } } = useForm<ServiceProvider>({
    defaultValues: serviceProviderDetails,
  });

  const { register: registerPassword, handleSubmit: handleSubmitPassword, reset: resetPassword, formState: { errors: errorsPassword } } = useForm<IPassword>();
      ...
      <div className="bg-gray-100 min-h-screen p-6">
        <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
          <div className="bg-gradient-to-r from-gray-900 to-blue-500 p-6 text-white">
            ...
            <button className="mt-4 inline-flex items-center text-sm font-medium text-gray-300 hover:text-white" onClick={() => window.history.back()}>
          ...
          <form onSubmit={handleSubmit(handleEdit)} className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="relative">
                ...
                <input
                  type="text"
                  id="name"
                  {...register('name', { required: "Name is required", validate: { notWhitespace: value => value.trim() !== "" || "Name cannot be empty" } })}
              ...
              <div className="relative">
                <label htmlFor="exp_year" className="block text-sm font-medium text-gray-700">Years of Experience</label>
                <input
                  type="number"
                  id="exp_year"
                  {...register('exp_year', { required: 'Experience is required' })}
                  className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300"
                />
                {errors.exp_year && <p className="text-sm text-red-600">{errors.exp_year.message}</p>}
              </div>

              <div className="relative">
                <label htmlFor="rate" className="block text-sm font-medium text-gray-700">Rate per session</label>
                <input
                  type="number"
                  id="rate"
            ...
            <div className="flex justify-end">
              <button type="submit" className="flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 focus:outline-none">
          ...
          <form onSubmit={handleSubmitPassword(handlePassword)} className="p-6 space-y-6">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300"
              />
              {errorsPassword.oldPassword && <p className="text-sm text-red-600">{errorsPassword.oldPassword.message}</p>}
            </div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300"
              />
              {errorsPassword.newPassword && <p className="text-sm text-red-600">{errorsPassword.newPassword.message}</p>}
            </div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300"
              />
              {errorsPassword.confirmPassword && <p className="text-sm text-red-600">{errorsPassword.confirmPassword.message}</p>}
            </div>
