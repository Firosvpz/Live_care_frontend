...
const AddSlot = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (serviceProviderInfo && !serviceProviderInfo.is_approved) {
      ...
      // Pre-fill the main category and fee if serviceProviderInfo is available
  ...
  useEffect(() => {
    // Only proceed if domainsList and serviceProviderInfo are available
    if (domainsList.length > 0 && serviceProviderInfo) {
        console.log('select',domainsList);
